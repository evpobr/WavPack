...
{
    WavpackContext *wpc = new(std::nothrow) WavpackContext();
...
int WavpackSetConfiguration64 (WavpackContext *wpc, WavpackConfig *config, int64_t total_samples, const unsigned char *chan_ids)
{
    ...
    for (wpc->current_stream = 0; num_chans; wpc->current_stream++) {
        WavpackStream *wps = new(std::nothrow) WavpackStream ();
        unsigned char left_chan_id = 0, right_chan_id = 0;
        int pos, chans = 1;

        // allocate the stream and initialize the pointer to it
        WavpackStream *new_streams = new(std::nothrow) *WavpackStream[wpc->current_stream + 1];
        std::copy(wpc->streams, wpc->streams + wpc->num_streams, new_streams);
        delete[] wpc->streams;
        wpc->streams = &new_streams;
...
int WavpackSetChannelLayout (WavpackContext *wpc, uint32_t layout_tag, const unsigned char *reorder)
{
    ...
    if (nchans && reorder) {
        ...
        wpc->channel_reordering = new (std::nothrow) unsigned char [nchans];
...
int WavpackPackInit (WavpackContext *wpc)
{
    ...
    for (wpc->current_stream = 0; wpc->current_stream < wpc->num_streams; wpc->current_stream++) {
        ...
        wps->sample_buffer = new(std::nothrow) int32_t[wpc->max_samples * (wps->wphdr.flags & MONO_FLAG ? 1 : 2)];
...
static int create_riff_header (WavpackContext *wpc, int64_t total_samples, void *outbuffer)
{
    ...
    char *outptr = reinterpret_cast<char *>(outbuffer);
...
static int pack_streams (WavpackContext *wpc, uint32_t block_samples)
{
    ...
    out2buff = (wpc->wvc_flag) ? new(std::nothrow) unsigned char [max_blocksize] : NULL;
    out2end = out2buff + max_blocksize;
    outbuff = new(std::nothrow) unsigned char [max_blocksize];
...
void WavpackUpdateNumSamples (WavpackContext *wpc, void *first_block)
{
    ...
    block_update_checksum (reinterpret_cast<unsigned char *>(first_block));
...
static void *find_metadata (void *wavpack_block, int desired_id, uint32_t *size)
{
    WavpackHeader *wphdr = reinterpret_cast<WavpackHeader *>(wavpack_block);
...
static int write_metadata_block (WavpackContext *wpc)
{
    ...
    if (wpc->metacount) {
        ...
        block_buff = new(std::nothrow) char [block_size + 6];
        wphdr = reinterpret_cast<WavpackHeader *>(block_buff);
